...
- **`app/travels/webapp/`** — the Fiori Elements UI copied from the base app so you can browse and drive the extension from the browser at <http://localhost:4006/travels/webapp/index.html>. The UI annotations themselves already sit inside `.base/index.csn`, so only the HTML/JS shell needs to travel with the template.
...
With option A's `cds watch` running, save the file. `cds watch` reloads. Open [xstravels-ext/test/requests.http](../xstravels-ext/test/requests.http) with the REST Client extension and send the requests:
...
{
  "error": {
    "message": "Total price 10322.5 exceeds the 10000 policy limit — cannot submit for review.",
    "code": "409",
...
**Rule 2b (per-booking cap)** — pick a travel whose **total is under 10 000 but one of its bookings is > 5 000**. In the default seed data, Travel 100 is a good candidate — it has a booking of 5 852 with a total of 8 735, so it clears rule 1 and gets caught by 2b. Since it's usually seeded as `Rejected`, reopen it first:

```http
### Reopen Travel 100 back to Open
POST http://localhost:4006/odata/v4/travel/Travels(ID=100,IsActiveEntity=true)/TravelService.reopenTravel
...
POST http://localhost:4006/odata/v4/travel/Travels(ID=100,IsActiveEntity=true)/TravelService.submitForReview
Authorization: Basic alice:
Content-Type: application/json
...
**Rule 1 (total-price cap)** — Travel 2 (Total = 10 322) trips this one independently.
...
- Travel 2 (USD, Total 10 322) → rejected against USD `MaxTravel` 10 000.
- Travel 13 (USD, Total 858) → passes.
- Travel 182 (SGD, Total 11 963) → **passes** despite being over 10 000 — because SGD `MaxTravel` is 14 000. This is the whole point of moving the caps into data.
