...
/**
 * update-dates — refresh xstravels seed data so the running app shows
 * a usable mix of past and upcoming flights + coherent travels/bookings.
 *
 * This script merges what used to be two scripts (xflights' update-flights-dates
 * and xtravels' update-travel-dates) into one, since xstravels contains both
 * datasets in one package.
 *
 * Order of operations:
 *   1. Rewrite the FlightSchedules CSV so every flight schedule row gets a
 *      random date in [today − 60, today + 90]. About one third land in the
 *      past bucket, two thirds in the future.
 *   2. Load the (now fresh) schedule and partition every entry into
 *      past / future buckets.
...
const FLIGHTS     = path.join(DATA_DIR, 'sap.capire.travels.flights-FlightSchedules.csv')
...
// ==========================================================================
// PART 1 — Refresh the flight schedule
// ==========================================================================

const flights = loadCSV(FLIGHTS)
...
for (const row of flights.records) {
  const id = row[flightIdCol]
...
for (const [, rows] of rowsByFlight) {
...
saveCSV(FLIGHTS, flights.header, flights.records)

console.log(`Updated ${flights.records.length} flight schedules:`)
console.log(`  past   : ${pastCount}  (${(pastCount   / flights.records.length * 100).toFixed(1)}%)`)
console.log(`  future : ${futureCount} (${(futureCount / flights.records.length * 100).toFixed(1)}%)`)

// ==========================================================================
// PART 2 — Rebase Travels + Bookings against the fresh schedule
// ==========================================================================
...
if (!allPast.length || !allFuture.length) {
  console.error('ERROR: schedule has no past or no future entries after refresh — bailing out.')
  process.exit(1)
}
...
function pickFlight(originalFlightId, bucket, targetCurrency) {
  ...
  // Prefer flights whose currency matches targetCurrency — the app's value-help
...
for (const t of travels.records) {
  ...
  for (const b of myBookings) {
    ...
    // Mirror the app's value-help behaviour: FlightPrice + Currency_code
    // are copied from the chosen flight row.
...
// ==========================================================================
// PART 3 — Recompute Travels.TotalPrice
// ==========================================================================
...
for (const t of travels.records) {
  ...
  const flightSum = flightSumByTravel.get(t.ID) || 0
  const suppSum   = supplementSumByTravel.get(t.ID) || 0
  const total = fee + flightSum + suppSum
...
console.log()
console.log('NOTE: The CSVs under db/data/ have been rewritten in place.')
console.log('These changes are for your local development only — please do NOT commit them.')
